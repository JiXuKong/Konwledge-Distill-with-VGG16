...
def run_testing(sess, ep):
    ...
    for it in range(10):
        ...
        loss += loss_ / 10.0
        acc += acc_ / 10.0
    summary = tf.Summary(value=[tf.Summary.Value(tag="test_total_loss", simple_value=loss), 
...
def VGG_16(in_put):
    ...
    with tf.variable_scope('vgg_16'):
        ...
        output = tf.nn.relu(batch_norm(conv2d(in_put, W_conv1_1) + b_conv1_1))
...
def student_network(in_put):
    output = Network(image = in_put, class_num = 10)
    return output
...
if __name__ == '__main__':
    ...
    teacher_output = VGG_16(x)
    student_output = student_network(x)
    
    T = 20
    a = 0.7
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=teacher_output))
    teacher_l2 = tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables(scope = 'vgg_16')])
    student_l2 = tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables(scope = 'classfier3x3')])
    l2 = teacher_l2 * weight_decay + student_l2*weight_decay*0.1
    kd_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=teacher_output/T, logits=student_output/T))
    total_loss = cross_entropy*(1-a) + a*kd_loss + l2
    ...
    correct_prediction = tf.equal(tf.argmax(student_output, 1), tf.argmax(y_, 1))
    ...
    with tf.Session() as sess:
        ...
        for ep in range(1, total_epoch+1):
            ...
            for it in range(1, iterations+1):
                ...
                _, batch_total_loss,  batch_cross_entropy, batch_kd_loss, batch_l2 = sess.run([train_step, total_loss, cross_entropy, kd_loss, l2],
                ...
                if it == iterations:
                    train_total_loss /= iterations
                    train_acc /= iterations


                    loss_, acc_ = sess.run([total_loss, accuracy],
                                           feed_dict={x: batch_x, y_: batch_y, keep_prob: 1.0, train_flag: True})
                    train_summary = tf.Summary(value=[tf.Summary.Value(tag="train_total_loss", simple_value=train_total_loss), 
                                                      tf.Summary.Value(tag="train_cross_entropy", simple_value=train_cross_entropy),
                                                      tf.Summary.Value(tag="train_kd_loss", simple_value=train_kd_loss),
                    ...
                    print("iteration: %d/%d, cost_time: %ds, train_loss: %.4f, "
                          ...
                          % (it, iterations, int(time.time()-start_time), train_total_loss, train_acc, val_loss, val_acc))
                else:
                    print("iteration: %d/%d, train_loss: %.4f, train_acc: %.4f"
                          % (it, iterations, train_total_loss / it, train_acc / it))
